  ...
  if (docs) {
    docs.forEach(doc => {
      ...
      if (
        ...
        let slug = formatPermalink(reference).startsWith('/') ? formatPermalink(reference).slice(1) : formatPermalink(reference)
        destination = `${process.env.NEXT_PUBLIC_SITE_URL}/${slug}`
      }
