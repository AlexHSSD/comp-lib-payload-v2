...
export type ComparisonBlockProps = Extract<
  Page["layout"][0],
  { blockType: "comparisonBlock" }
>;

export const ComparisonBlock: React.FC<ComparisonBlockProps & blockNum> = ({
  blockNum,
  comparisonBlockFields: { title, richText, columns, tableHeading },
  blockName = "ComparisonBlock",
}) => {
  const questions = Array.from(
    new Set(columns.flatMap((test) => test.rows).map((row) => row.name))
  );
        ...
        <div className="flex">
          <div
            className={`flex-1 max-md:max-w-[50%] ${
              columns.length > 3 ? "md:max-w-[33.333%]" : "mx:max-w-[50%]"
            }`}
          >
