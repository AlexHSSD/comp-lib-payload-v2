...
export type BlogArchiveBlockProps = Extract<
  Page["layout"][0],
...
const getURL = (relationTo, slug, breadcrumbs) => {
  ...
  if (relationTo === "blogs") {
    const hasBreadcrumbs =
      breadcrumbs && Array.isArray(breadcrumbs) && breadcrumbs.length > 0;
    if (hasBreadcrumbs) {
      return ("/blogs" + breadcrumbs[breadcrumbs.length - 1]?.url) as string;
    }
  }
...
export const BlogArchiveBlock: React.FC<BlogArchiveBlockProps & blockNum> = (
  props
) => {
  const {
